...
│   │   ├── AppLayout.tsx     # Wrapper: Sidebar + Main + Footer (без Header)
│   │   ├── Sidebar.tsx       # Основная навигация: сворачиваемый sidebar с логотипом, профилем и logout
│   │   └── Footer.tsx        # Футер
│   │
...
Все страницы рендерятся внутри `<AppLayout>`. Sidebar и футер **одни на всё приложение** и никогда не пересоздаются при навигации. **Header отсутствует** — вся навигация в Sidebar.
...
### 3.2 Sidebar — основная навигация

```
Развёрнутый (w-64):          Свёрнутый (w-16):
┌──────────────────┐         ┌────┐
│ EduQuest    [◀]  │         │[▶] │
├──────────────────┤         ├────┤
│ 🏠 Главная       │         │ 🏠 │
...
- **Десктоп**: фиксированная боковая панель, можно свернуть в режим иконок (w-16) кнопкой-стрелкой; состояние `sidebarCollapsed` сохраняется в localStorage
- **Мобильные**: скрыта, hamburger-кнопка (fixed top-left) открывает sidebar поверх контента с overlay
- Содержит: логотип, навигацию по ролям, профиль и кнопку выхода внизу
- Навигация адаптируется по роли (ADMIN видит "Управление", TEACHER видит "Мои курсы", STUDENT — все основные пункты)
- Текущая страница подсвечивается активным стилем
- Состояние хранится в `ui.store.ts` (sidebarOpen для мобильных, sidebarCollapsed для десктопа)
...
- [ ] Все страницы рендерятся внутри `<AppLayout>` — один Sidebar на всё приложение (Header отсутствует)
- [ ] Один компонент `Sidebar` — вся навигация, профиль и logout внутри; адаптируется по роли
- [ ] На десктопе sidebar сворачивается/разворачивается кнопкой (иконки / полное меню)
- [ ] На мобильных hamburger-кнопка открывает **тот же Sidebar** поверх контента с overlay
...
- [ ] Максимальнаря ширина контента: `max-w-7xl mx-auto`
